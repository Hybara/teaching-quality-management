...
$(function () {
  ...
  $(".readAllMessage").on("click", function (event) {
    ...
    if (confirm("这将把所有公告标记为已读，您确认要这么做？")) {
      var token = $("body").attr("data-token");
      $.post("/teacher/readAllMessages", {token: token}, function (response) {
        if (response == "ok") {
          alert("操作成功");
          window.location.href = "/teacher/goMessage/" + token;
...
function initMessagePanel($item, message, token) {
  ...
  $item.find("a.del").attr("href", "/teacher/delMessage/");
}
