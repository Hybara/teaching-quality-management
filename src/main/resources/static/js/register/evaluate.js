...
$(function () {
  ...
  $("form").on("submit", function (event) {
    ...
    $.post("/register/evaluate", data, function (response) {
      if (response == "ok") {
        window.location.href = '/register/goEvaluate/'+majorId+"/"+teacherId+"/"+scoreId+"/"+token;
...
function initEvaluateList(type, page) {
  ...
  $.post("/register/getScoreEvaluates", {
    token: token,
    scoreForClassId: $("li.score").attr("data-id"),
    type: type,
    page: page
  }, function (response) {
    evaluate_type = type;
