$(function () {
  ...
  $("#old_password").on("blur", function () {
    ...
    if (password==undefined || password==null || password=="") {
      $($(this).siblings("span").get(0)).text("请输入您的当前密码").addClass("text-danger").removeClass("text-success");
      $(this).focus();
      return ;
    }
    console.log("11");
    var $input = $(this);
    $.post("/student/checkPassword", {
      token: token,
      password: password
    }, function (response) {
      if (response=="ok") {
        $($input.siblings("span").get(0)).text("密码正确").addClass("text-success").removeClass("text-danger");
      } else if(response=="error") {
        $($input.siblings("span").get(0)).text("密码错误，请重新输入").addClass("text-danger").removeClass("text-success");
        $input.val("").focus();
